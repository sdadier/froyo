...
void update_osr128(bool with_mute)
{
	...
	bypass_write_hook = true;
//	printk("--------> wm8994 : osr128 - oversampling : %d\n", val);
...
void update_dac_direct(bool with_mute)
{
	...
	bypass_write_hook = true;
	val1 &=  ~(WM8994_DAC1L_TO_HPOUT1L_MASK | WM8994_DAC1L_TO_MIXOUTL_MASK);
	val2 &= ~(WM8994_DAC1R_TO_HPOUT1R_MASK | WM8994_DAC1R_TO_MIXOUTR_MASK);
...
#ifdef CONFIG_SND_VOODOO_HP_LEVEL_CONTROL
//		printk("-------> voodoo_hook_wm8994_write reg = %d, value = %d\n", reg, value);
		if (is_path(HEADPHONES)
		    && !(wm8994->codec_state & CALL_ACTIVE)) {
		...
		if (reg == WM8994_FLL1_CONTROL_4)  // XSD WM8994_FLL1_CONTROL_4
